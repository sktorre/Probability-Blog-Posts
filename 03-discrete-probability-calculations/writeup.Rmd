...
#Calculate the probability of winning the world series acrooss a range of different possible game probabilities
...
series %>% ggplot(aes(x = games, y = p)) +
  ...
  geom_text(aes(x = total_games - 2, y = p_win +  0.015, label = "73")) +
...
```{r}
win <- c(dnbinom(3,4,0.55)/0.5,
         dnbinom(3,4,0.55),
         dnbinom(3,4,0.55)/(dnbinom(3,4,0.55) +  dnbinom(3,4,0.45)),
         dnbinom(3,4,0.45)/0.5,
         dnbinom(3,4,0.45),
         dnbinom(3,4,0.45)/(dnbinom(3,4,0.55) +  dnbinom(3,4,0.45))
        )

not_win <- c((0.5-dnbinom(3,4,0.55))/0.5,
             0.5-dnbinom(3,4,0.55),
             (0.5-dnbinom(3,4,0.55))/0.5/((0.5-dnbinom(3,4,0.55)) + (0.5-dnbinom(3,4,0.45))),
             (0.5-dnbinom(3,4,0.45))/0.5,
             0.5-dnbinom(3,4,0.45),
             (0.5-dnbinom(3,4,0.45))/((0.5-dnbinom(3,4,0.55)) + (0.5-dnbinom(3,4,0.45)))
              )

table_values <- data.frame(win,not_win)

col_totals <- c(dnbinom(3,4,0.55) +  dnbinom(3,4,0.45),
                (0.5-dnbinom(3,4,0.55)) +
                (0.5-dnbinom(3,4,0.45))
                )
```
...
We know *P*(*P*<sub>*B*</sub> = 0.55\|Braves win World Series in 7 games) is the column probability for the top left section which can be found by the cell probability divided by the total probability that the Braves win in exactly 7 games (0.16677/0.29). This means that *P*(*P*<sub>*B*</sub> = 0.55\|Braves win World Series in 7 games) = 0.5714. 
...
$P(P_B = 0.55|Braves win World Series in 7 Games)=\frac{P(Braves win World Series in 7 Games|P_B = 0.55)P(P_B = 0.55)}{P(Braves win World Series in 7 Games)} = \frac{0.3335*0.5}{0.29187} = 0.571$ 
